...
def main(argv=None):
    ...
    if sys.platform.startswith("win"):
        sources = set(
            [source for option in options.input for source in glob(escape(option))]
        )
    else:
...
def buildHTML(path, imgfile, imgfilepath):
    ...
    if not options.noprocessing and "Rotated" in options.imgMetadata[imgfilepath]:
        ...
        rotatedPage = False
    if (
        not options.noprocessing
        and "BlackBackground" in options.imgMetadata[imgfilepath]
    ):
    ...
    f.writelines(
        [
            ...
            '<div style="text-align:center;top:'
            + getTopMargin(deviceres, imgsizeframe)
            + '%;">\n',
            '<img width="'
            + str(imgsizeframe[0])
            + '" height="'
    ...
    if options.iskindle and options.panelview:
        ...
        boxStyles = {
            "PV-TL": "position:absolute;left:0;top:0;",
        ...
        for i in range(0, len(boxes)):
            f.writelines(
                [
                    ...
                    'data-app-amzn-magnify=\'{"targetId":"'
                    + boxes[i]
                    + '-P", "ordinal":'
                    + str(order[i])
                    + "}'></a>\n",
...
def buildNCX(dstdir, title, chapters, chapternames):
    ...
    for chapter in chapters:
        folder = (
            chapter[0]
            .replace(os.path.join(dstdir, "OEBPS"), "")
            .lstrip("/")
            .lstrip("\\\\")
        )
...
def buildOPF(dstdir, title, filelist, cover=None):
    ...
    f.writelines(
        [
            ...
            options.uuid,
            "</dc:identifier>\n",
            '<dc:contributor id="contributor">KindleComicConverter-'
            + __version__
            + "</dc:contributor>\n",
        ]
    )
    ...
    if cover is not None:
        filename = getImageFileName(
            cover.replace(os.path.join(dstdir, "OEBPS"), "").lstrip("/").lstrip("\\\\")
        )
        if ".png" == filename[1]:
            mt = "image/png"
        else:
    ...
    for path in filelist:
        ...
        uniqueid = (
            os.path.join(folder, filename[0]).replace("/", "_").replace("\\", "_")
        )
...
def buildEPUB(path, chapternames, tomenumber):
    ...
    for dirpath, dirnames, filenames in os.walk(os.path.join(path, "OEBPS", "Images")):
        ...
        for afile in filenames:
            ...
            if cover is None:
                ...
                options.covers.append(
                    (
                        image.Cover(
                            os.path.join(filelist[-1][0], filelist[-1][1]),
                            cover,
                            options,
                            tomenumber,
                        ),
    ...
    if not chapternames and options.chapters:
        ...
        for aChapter in options.chapters:
            ...
            chapterlist.append(
                (filelist[pageid][0].replace("Images", "Text"), filename)
            )
...
def imgDirectoryProcessing(path):
    ...
    if len(work) > 0:
        for i in work:
            workerPool.apply_async(
                func=imgFileProcessing, args=(i,), callback=imgFileProcessingTick
            )
        workerPool.close()
        workerPool.join()
        ...
        if len(workerOutput) > 0:
            rmtree(os.path.join(path, "..", ".."), True)
            raise RuntimeError(
                "One of workers crashed. Cause: " + workerOutput[0][0],
                workerOutput[0][1],
            )
    ...
    if wantedname is not None:
        if wantedname.endswith(ext):
            ...
            filename = os.path.join(
                os.path.abspath(options.output), os.path.basename(srcpath) + ext
            )
        else:
            filename = os.path.join(
        ...
        if "Ko" in options.profile and options.format == "EPUB":
            ...
            path[-1] = (
                "".join(e for e in path[-1].split(".")[0] if e.isalnum())
                + tomenumber
                + ext
            )
...
def sanitizeTree(filetree):
    ...
    for root, dirs, files in os.walk(filetree, False):
        for name in files:
            ...
            while (
                os.path.exists(os.path.join(root, slugified + splitname[1]))
                and splitname[0].upper() != slugified.upper()
            ):
                slugified += "A"
        ...
        for name in dirs:
            ...
            while (
                os.path.exists(os.path.join(root, slugified))
                and name.upper() != slugified.upper()
            ):
                slugified += "A"
    ...
    for root, _, files in os.walk(os.path.join(path, "OEBPS", "Images")):
        for f in files:
            if (
                ...
                newLevel = (
                    os.path.join(root, f)
                    .replace(os.path.join(path, "OEBPS", "Images"), "")
                    .count(os.sep)
                )
...
def detectCorruption(tmppath, orgpath):
        ...
        for name in files:
            if getImageFileName(name) is not None:
                ...
                try:
                    ...
                    imageNumber += 1
                    if (
                        options.profileData[1][0] > img.size[0]
                        and options.profileData[1][1] > img.size[1]
                    ):
                        imageSmaller += 1
                    ...
                    if "decoder" in str(err) and "not available" in str(err):
                        raise RuntimeError(
                            "Pillow was compiled without JPG and/or PNG decoder."
                        )
                    else:
                        raise RuntimeError(
                            "Image file %s is corrupted. Error: %s"
                            % (pathOrg, str(err))
                        )
            else:
    ...
    if alreadyProcessed:
        print(
            "WARNING: Source files are probably created by KCC. The second conversion will decrease quality."
        )
        if GUI:
            GUI.addMessage.emit(
                "Source files are probably created by KCC. The second conversion will decrease quality.",
                "warning",
                False,
            )
            GUI.addMessage.emit("", "", False)
    if (
        imageSmaller > imageNumber * 0.25
        and not options.upscale
        and not options.stretch
    ):
        print(
...
def makeParser():
    psr = ArgumentParser(
        prog="kcc-c2e", usage="kcc-c2e [options] [input]", add_help=False
    )
    ...
    custom_profile_options.add_argument(
        "--customwidth",
        type=int,
        dest="customwidth",
        default=0,
...
def checkOptions(options):
    ...
    if options.format == "EPUB-50MB":
        options.targetsize = 45
        options.format = "EPUB"
    ...
    if (
        options.format == "MOBI" or options.format == "KFX"
    ) and options.batchsplit != 2:
        options.batchsplit = 1
    # Older Kindle models don't support Panel View.
    if (
        options.profile == "K1"
        or options.profile == "K2"
        or options.profile == "K34"
        or options.profile == "KDX"
    ):
    ...
    if options.profile == "KS" and (
        options.format == "MOBI" or options.format == "EPUB"
    ):
...
def checkTools(source):
    ...
    if (
        source.endswith(".CB7")
        or source.endswith(".7Z")
        or source.endswith(".RAR")
        or source.endswith(".CBR")
        or source.endswith(".ZIP")
        or source.endswith(".CBZ")
    ):
...
def makeBook(source, qtgui=None):
    ...
    for tome in tomes:
        ...
        if len(tomes) > 9:
            ...
            tomeNumber += 1
            options.title = (
                options.baseTitle + " [" + str(tomeNumber) + "/" + str(len(tomes)) + "]"
            )
        if options.format == "CBZ":
    ...
    if not GUI and options.format == "MOBI":
        ...
        for i in filepath:
            ...
            if k.path and k.coverSupport:
                options.covers[filepath.index(i)][0].saveToKindle(
                    k, options.covers[filepath.index(i)][1]
                )
    if options.delete:
...
def makeMOBIFix(item, uuid):
    ...
    try:
        dualmetafix.DualMobiMetaFix(
            mobiPath + "_toclean", mobiPath, bytes(uuid, "UTF-8")
        )
        return [True]
    except Exception as err:
...
def makeMOBIWorker(item):
    ...
    try:
        if os.path.getsize(item) < 629145600:
            output = Popen(
                'kindlegen -dont_append_source -locale en "' + item + '"',
                stdout=PIPE,
                stderr=STDOUT,
                stdin=PIPE,
                shell=True,
...
def makeMOBI(work, qtgui=None):
    ...
    for i in work:
        makeMOBIWorkerPool.apply_async(
            func=makeMOBIWorker, args=(i,), callback=makeMOBIWorkerTick
        )
