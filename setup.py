...
class BuildBinaryCommand(distutils.cmd.Command):
    ...
    def run(self):
        ...
        if sys.platform == "darwin":
            ...
            os.system(
                'pyinstaller -y -F -i icons/comic2ebook.icns -n "Kindle Comic Converter" -w -s kcc.py'
            )
            os.makedirs("dist/Kindle Comic Converter.app/Contents/Resources/Codecs")
            shutil.copy(
                "other/osx/7z", "dist/Kindle Comic Converter.app/Contents/Resources"
            )
            shutil.copy(
                "other/osx/7z.so", "dist/Kindle Comic Converter.app/Contents/Resources"
            )
            shutil.copy(
                "other/osx/Rar.so",
                "dist/Kindle Comic Converter.app/Contents/Resources/Codecs",
            )
            shutil.copy(
                "other/osx/Info.plist", "dist/Kindle Comic Converter.app/Contents"
            )
            shutil.copy(
                "LICENSE.txt", "dist/Kindle Comic Converter.app/Contents/Resources"
            )
            shutil.copy(
                "other/windows/Additional-LICENSE.txt",
                "dist/Kindle Comic Converter.app/Contents/Resources",
            )
            os.chmod("dist/Kindle Comic Converter.app/Contents/Resources/7z", 0o777)
            # TODO /usr/bin/codesign --force -s "$MACOS_CERTIFICATE_NAME" --options runtime dist/Applications/Kindle\ Comic\ Converter.app -v
            os.system(
                "appdmg kcc.json dist/KindleComicConverter_osx_" + VERSION + ".dmg"
            )
            exit(0)
        elif sys.platform == "win32":
            os.system(
                "pyinstaller -y -F -i icons\\comic2ebook.ico -n KCC_"
                + VERSION
                + " -w --noupx kcc.py"
            )
            exit(0)
            ...
            os.system(
                "pyinstaller --hidden-import=queue -y -F -i icons/comic2ebook.ico -n kcc_linux_"
                + VERSION
                + " kcc.py"
            )
            exit(0)
